...
function searchedTaskTable (inputTxt, elPosTask, itemKeys) {
  if (elPosTask !== -1 && inputTxt.length > 0 && ((!document.querySelector("#searchedProjDiv").classList.contains("displayBlock") && !document.querySelector("#searchedTaskDiv").classList.contains("displayBlock")))) {
    $(".tbTaskRow").empty();
    ...
    const taskSelectedValues = Object.values(allStorage()[2][elPosTask]);
  ...
  } else if (elPosTask !== -1 && inputTxt.length > 0 && ((!document.querySelector("#searchedProjDiv").classList.contains("displayBlock") && document.querySelector("#searchedTaskDiv").classList.contains("displayBlock")))) {  
    $('#content21').css({
        'grid-column': '1 / 2', 
        'grid-row': '1 / 2'
    });
...
function Content22 () {
  ...
  const searchTaskBtnFunc = (e) => {
    ...
    if (taskNames.indexOf(currentInputValue) > -1 && currentInputValue.length > 0) {
      ...
      searchedTaskTable(inputText, elementPosTask, objKeys); 
    } else {
  ...
  const searchProjBtnFunc = (e) => {
    ...
    if (projNames.indexOf(currentInputValue) > -1 && currentInputValue.length > 0) {
      ...
      searchedProjTable (inputText, elementPosProj, objKeys);
      dragDropTasks();
    } else {
      setSearchTaskProjModal(true);
    }
